...
By default, three kinds of output files are generated by LQC: summary
tables, figures, and an HTML report. With the `--output-cs` option, the
processed cs tag information of the BAM file is also written. Similarly,
with the `--output-pickle` option, the result objects are written to one
pickle file, which can be used for further analysis.
