...
def cigar_to_list(cigar_string):
    '''convert the cigar_strings into a list with values and genomic positions (0-based) [low, high, cigar tag, cigar value].'''
    # 0 based
    pos = 0
    cigarlenfuncs = {
        'M': int,
...
def convert_cigar_md_to_cs_list(cigar_string,
    ...
    for _, _, cigar_mark, cigar_value in cm:
        if cigar_mark == 'M':
            ...
            seq_pos += 1
        elif cigar_mark == 'I' or cigar_mark == 'S':
            seq_pos += int(cigar_value)
        else:
            pass
        # convert to 0-base
    ...
    for i in range(len(new_cs)):
        if new_cs[i][2] == '~':
            ...
            new_val = f'{split1}{old_val}{split2}'
            new_cs[i][3] = new_val
        else:
            pass
    return new_cs
...
class CS:
    ...
    def get_read_location(self):
        if self._read_location is None:
    ...
    def get_normalized_read_location(self):
        if self._normalized_read_location is None:
            read_loc = self.get_read_location()
            read_len = self.get_read_length()
            self._normalized_read_location = [
                [a / read_len, b / read_len, c, d]
                for a, b, c, d in read_loc
            ]
        return self._normalized_read_location
    ...
    def __str__(self):
        outstring = f'CS tag: {self._intron_count} introns, {self._mismatch_count} mismatches, {self._insertion_count + self._deletion_count} indels'
        return outstring

    def __repr__(self):
        outstring = '\n'.join([
            "CS tag:",
            f"  {self._intron_count} introns",
            f"  {self._mismatch_count} mismatches",
            f"  {self._insertion_count + self._deletion_count} indels",
            "--------------------",
